...
const tips = {
  ...
  tcpMux: '是否开启 yamux 多路复用。true=所有控制/工作连接共享 1 条 TCP；false=每条连接独立 TCP。默认 true。<br/><br/>TCPMux 模式下，yamux 每 30s 发送一次 ping 保活，只要 yamux session 存活，所有 work conn stream 就不会被 NAT/防火墙断开。关闭后每条连接建立后完全静默，无任何应用层保活，需依赖 OS TCP keepalive。',
  ...
  tlsServerName: 'TLS SNI（Server Name Indication）服务器名称。用于 TLS 握手时的域名验证。留空则使用 serverAddr 的值。仅 QUIC 模式可用。',
  ...
  webUser: 'Dashboard 管理面板登录用户名。搭配下方密码使用。仅在配置了 webServer 端口（默认 7400）时生效。',
